...
impl PtyManager {
    ...
    pub fn get_writer(&self, id: &str) -> Result<Arc<Mutex<Box<dyn Write + Send>>>, String> {
        let instance = self.instances.get(id).ok_or_else(|| format!("PTY {} not found", id))?;
        Ok(instance.writer.clone())
    }

    pub fn get_master(&self, id: &str) -> Result<Arc<Mutex<Box<dyn MasterPty + Send>>>, String> {
        let instance = self.instances.get(id).ok_or_else(|| format!("PTY {} not found", id))?;
    ...
    E: FnOnce() + Send + 'static,
{
    ...
    // OSC 7 = current directory for CWD tracking
    // OSC 133;A = prompt start mark for command zone navigation
    // OSC 133;B = prompt end mark
    let prompt_hook = r#"function prompt {
  $loc = (Get-Location).Path
  $Host.UI.RawUI.WindowTitle = $loc
  $e = [char]27
  $hostname = $env:COMPUTERNAME
  $urlpath = $loc.Replace('\', '/').Replace(' ', '%20')
  if ($urlpath -notmatch '^/') { $urlpath = '/' + $urlpath }
  # OSC 7 (cwd) + OSC 133 A (prompt start)
  "$e]7;file://$hostname$urlpath$e\$e]133;A$e\PS $loc$('>' * ($nestedPromptLevel + 1)) $e]133;B$e\"
}"#;
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            let valid_len = {
                ...
                // Walk back up to 3 bytes to find a valid boundary
                while i > 0 && i > pending.len().saturating_sub(4) {
                    if std::str::from_utf8(&pending[..i]).is_ok() {
                        break;
                    }
                    i -= 1;
                }
                if i == 0 && !pending.is_empty() {
                    // Fallback: send everything lossy rather than stalling
                    pending.len()
                } else {
                    i
                }
            ...
            if valid_len > 0 {
                let text = if valid_len == pending.len() {
                    // Fast path: all bytes are valid UTF-8
